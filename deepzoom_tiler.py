    ...
    def _write_tiles(self):
        ...
        for level in range(self._dz.level_count):
            if not (level in target_levels):
                continue
            tiledir = os.path.join(self._basename, str(mag_list[mag_idx]))
...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg', temp_dir='WSI_temp_files'):
    print('\n Organizing patches')
    img_name = img_slide.split(os.sep)[-1].split('.')[0]
    img_class = img_slide.split(os.sep)[2]
    n_levels = len(glob.glob(os.path.join(temp_dir, '*')))
    bag_path = os.path.join(out_base, img_class, img_name)
    os.makedirs(bag_path, exist_ok=True)
    if len(level)==1:
        patches = glob.glob(os.path.join(temp_dir, '*', '*.' + ext))
        ...
        levels = [int(os.path.basename(i)) for i in glob.glob(os.path.join(temp_dir, '*'))]
        levels.sort()
        low_patches = glob.glob(os.path.join(temp_dir, str(levels[0]), '*.'+ext))
        for i, low_patch in enumerate(low_patches):
            ...
            for x_pos in high_x_list:
                for y_pos in high_y_list:
                    high_patch = glob.glob(os.path.join(temp_dir, str(levels[1]), '{}_{}.'.format(x_pos, y_pos)+ext))
                    if len(high_patch)!=0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--background_t', type=int, default=35, help='Threshold for filtering background [15]')  
    args = parser.parse_args()
    levels = tuple(sorted(args.magnifications))
    assert len(levels) <= 2, 'Only 1 or 2 magnifications are supported!'
    # 修改为使用 /root/public/datasets 下的特定数据集路径
    path_base = os.path.join('/home/lin01231/public/datasets', args.dataset)
    if len(levels) == 2:
        out_base = os.path.join('/home/lin01231/public/datasets', args.dataset, 'pyramid')
    else:
        out_base = os.path.join('/home/lin01231/public/datasets', args.dataset, 'single')
    ...
    for idx, c_slide in enumerate(all_slides):
        img_name = os.path.basename(c_slide).split('.')[0]
        img_class = c_slide.split(os.sep)[-2]
        out_path = os.path.join(out_base, 'lin01231', img_name)
        temp_dir = f"WSI_temp_files_{args.dataset}"

        if os.path.exists(out_path) and len(os.listdir(out_path)) > 0:
            print(f'Skip slide {idx+1}/{len(all_slides)}: {img_name} already processed.')
            continue

        print('Process slide {}/{}: {}'.format(idx + 1, len(all_slides), img_name))
        DeepZoomStaticTiler(
            c_slide, temp_dir, levels, args.base_mag, args.objective, args.format, 
            args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t
        ).run()
        nested_patches(c_slide, out_base, levels, ext=args.format, temp_dir=temp_dir)
